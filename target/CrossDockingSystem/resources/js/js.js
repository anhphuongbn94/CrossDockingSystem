...
$(document).ready(function(e){
	...
	$('.edit-outdoor').click(function(e){
		...
		var content="";
		content += "<td><input type='text' size='10' value='"+nameD+"' class='form-control nameD'></td>" 
				+ "<td><input type='text' size='3' value='"+capD+"' class='form-control capD'></td>"
				+ "<td>" 
	...
	$('.edit-indoor').click(function(e){
		...
		content += "<td><input type='text' value='"+nameD+"' class='form-control nameD'></td>" 
				...
				+ "<td>" 
					...
					+ "</select>" 
				+ "</td>"
				+ "<td><a class='save-indoor'>Save</a></td>";
		trC.html(content);
		$('.save-indoor').click(function(e){
			trC=$(this).parents('tr');
...
function insertInVehicle(){
	$.ajax({
        ...
        success: function (response) {
        	var msgErr='';
        	if(getCurrentLanguage() == "en"){
        		msgErr='Vehicle information entered already exists';
        	}else{
        		msgErr='Thông tin xe nhập đã tồn tại';
        	}
        	if(response == ''){
        		$('.form-insert-iv .msg-error').remove();
        		$('.form-insert-iv .cmxform').before("<section class='msg-error'>"+msgErr+"</section>");
        	}else{
        		$('.form-insert-iv .msg-error').remove();
        		showContentInVehicle(response);
        		currentPage = 1;
	...
	$.ajax({
        ...
        success: function (response) {
        	...
        	if(response == ''){
        		...
        		$('.form-insert-ov .cmxform').before("<section class='msg-error'>"+msgErr+"</section>");
        	}else{
        		showContentOutVehicle(response);
        		currentPage = 1;
        	}
